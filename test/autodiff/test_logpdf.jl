...
# Define the test body as a macro to allow ReTest to see the tests
macro run_tests_body(backend)
    return quote
...
@testset_macro @run_tests_body

@testset "ForwardDiff logpdf autodiff tests" begin
    @run_tests_body(AutoForwardDiff())
end

@testset "Zygote logpdf autodiff tests" begin
    @run_tests_body(AutoZygote())
end

if get(ENV, "GMRF_TEST_ENZYME", "false") == "true"
    @testset "Enzyme logpdf autodiff tests" begin
        @run_tests_body(AutoEnzyme(; function_annotation = Enzyme.Const))
    end
end
