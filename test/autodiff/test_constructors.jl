...
@testset "GMRF constructor autodiff tests" begin
    ...
    function compare_constructor_gradients(θ::Vector, k::Int; h::Float64 = 1.0e-6)
    ...
    @testset "Constructor autodiff with default algorithm" begin
        ...
        rel_error_zygote = abs_error_zygote ./ (abs.(grad_fd) .+ 1.0e-10)

        @test maximum(abs_error_zygote) < 1.0e-4
        @test maximum(rel_error_zygote) < 1.0e-2
    end
