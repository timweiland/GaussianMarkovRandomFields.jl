...
grid = generate_grid(Line, (Nₓ - 1,), Ferrite.Vec(x_left), Ferrite.Vec(x_right))
...
grid_time = generate_grid(Line, (Nₜ - 1,), Ferrite.Vec(t_start), Ferrite.Vec(t_stop))
...
A_initial = evaluation_matrix(disc, [Ferrite.Vec(x) for x in xs_initial])
t_initial_idx = 1 # Observe at first time point
A_initial = spatial_to_spatiotemporal(A_initial, t_initial_idx, Nₜ)
A_later = evaluation_matrix(disc, [Ferrite.Vec(x_later)])
t_later_idx = 2 * Nₜ ÷ 3
