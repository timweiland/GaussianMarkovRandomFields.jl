...
export create_inflated_rectangle, create_rectangle
...
Useful for FEM discretizations of SPDEs, where the domain is often artificially inflated
to avoid undesirable boundary effects.
...
- `boundary_width::Real`: width of the inflated boundary
...
function create_inflated_rectangle(
    ...
    # # boundary_tags, _ = gmsh.model.mesh.getNodesForPhysicalGroup(1, 2)
    # # boundary_tags = map(x -> Int(x), boundary_tags)
...
function create_rectangle(
    ...
    group = gmsh.model.addPhysicalGroup(2, [s_int])
    gmsh.model.setPhysicalName(2, group, "Domain")
    int_group = gmsh.model.addPhysicalGroup(2, [s_int])
    gmsh.model.setPhysicalName(2, int_group, "Interior")
    boundary_group = gmsh.model.addPhysicalGroup(1, [l_int_1, l_int_2, l_int_3, l_int_4])
